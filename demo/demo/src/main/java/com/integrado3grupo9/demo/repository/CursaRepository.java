...
public interface CursaRepository extends JpaRepository<Cursa, CursaId> {
    ...
    @Query("SELECT c FROM Cursa c WHERE c.id.dniEstudiante = :dniEstudiante AND c.id.IdCarrera")
    Cursa findByEmbeddedId(Long idCarrera, Long dniEstudiante);

    @Transactional
    @Modifying
    @Query("DELETE FROM Cursa c WHERE c.id.dniEstudiante = :dniEstudiante AND c.id.idCarrera")
